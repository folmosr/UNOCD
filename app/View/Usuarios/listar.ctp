...
<div class="col-md-12">
  <div class="box">
    <div class="box-title">
      <h3><i class="icon-table"></i> Usuarios Registrados</h3>
    </div>
          ...
          <div class="box bordered-box green-border" style="margin-bottom:0;">
			...
			?>
              <table class="table table-bordered table-hover" style="margin-bottom:0;">
                <tbody>
                 <?php 
				 	if(count($this->Session->read('Auth.User.Cliente')) > 0) {
				 ?> 
                  <tr>
                  	<td align="left" style="width:20%;text-align:left"><b>Clientes Asociados</b></td>
                    <td><?php echo $this->Form->input('Usuario.ncli', array(
											'label'=>false,
											'div'=>false,
											'options'=>$clientes,
											'empty'=>'[Seleccionar]',
              ...
              <?php 
				...
				<tr>
					...
					<td style="text-align:left"><?php echo $data[$i]['Usuario']['apellido'].' '.$data[$i]['Usuario']['nombre']; ?></td>                    
                 	...
                 	<td style="text-align:left"><?php echo $data[$i]['Cliente'][0]['nombre']; ?></td>
                    <td valign="middle">
                 	...
                 	<td>
                       <div class="btn-group">
                      		<a href="<?php  if($data[$i]['Usuario']['status'])  echo '/unocdcom/usuarios/editar/'.$this->CustomFunctions->encode($data[$i]['Usuario']['id']); else echo '#'; ?>" class="btn btn-circle <?php if($data[$i]['Usuario']['status']) echo 'btn-success'; ?>" title="Actualizar Datos de Usuario <?php if(!$data[$i]['Usuario']['status']) echo '- active al usuario para poder realizar esta acción sobre los datos del mismo'; ?>"><i class="icon-ok-sign"></i></a>                       
                            <a href="/unocdcom/usuarios/q_on/<?php echo $this->CustomFunctions->encode($data[$i]['Usuario']['id']); ?>" class="btn btn-circle <?php  echo ($data[$i]['Usuario']['status'])?'btn-danger':'btn-success'; ?>" title="Desactivar Usuario"><i class="icon-user"></i></a>
                      </div>
                    </td>
                 </tr>
              <?php 
