...
header('Content-Disposition: attachment;filename=Usuarios_UNOCD.xlsx');
...
$objPHPExcel->getProperties()->setCreator('UNOCD - 2014')
                             ...
                             ->setTitle('Office 2007 XLSX Reporte de Usuarios')
                             ->setSubject('Office 2007 XLSX Reporte de Usuarios')
                             ->setDescription('Reporte de Usuarios')
                             ->setKeywords('UNOCD Almacenadora Distribuidora Invetario Productos Pedidos Usuarios')
...
$objPHPExcel->getActiveSheet()->setCellValue('C2', 'REPORTE DE USUARIOS ');
...
for($i = 0; $i < count($data); $i++):
	...
	$objPHPExcel->getActiveSheet()->getStyle('F'.$row)->applyFromArray(
  	...
  	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row, $data[$i]['Cliente'][0]['nombre']); 
